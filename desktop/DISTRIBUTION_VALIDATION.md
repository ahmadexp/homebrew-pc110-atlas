...
| Pi-Apps candidate: checksum/OS guards, failure cleanup, dependency-helper contract, deterministic ZIP allowlist and permissions | 9 tests passed; ShellCheck and workflow actionlint passed |
| Pi-Apps real helper installation, packaged smoke, four UI renders, reinstall, uninstall, repeated uninstall, and synthetic media preservation | Passed locally in native ARM64 Debian Trixie and Ubuntu 24.04 containers; [details and limits](packaging/pi-apps/README.md) |
| Actual Raspberry Pi OS Trixie on Pi 5, 4 GB: installation, synthetic guest boot, keyboard and reset | Passed with a diagnostic FreeType preload for imports; [hardware test and limits](packaging/pi-apps/HARDWARE-TEST.md) |
| Private PersonaWare test after Pi recovery, unchanged native core and verified firmware | Not passed: original menu renders but input does not open apps; English image stalled at DOS startup; no-input diagnostic reproduced stopped CPU |
| Original 1.0.0 Pi native picker, PCB-layer layout and resize/PDF checks | Historical failures: default picker and layer-dialog failures; host became unreachable during PDF check after low-voltage warning. Reproduced app failures were subsequently fixed and retested in 1.0.1 below |
| Separate private Linux QEMU 1.0.1 fix candidate, Pi 5 Trixie | Reproduced picker, PCB layout, resize and guest execution failures resolved; built-in PDF viewer inspected; original and English PersonaWare booted and calculated 2 + 3 = 5; live mouse click opened WorldClock. See [retest](packaging/pi-apps/HARDWARE-TEST.md#linux-qemu-fix-retest-september-17-2026) for exact artifacts and limits |
| Private fix candidate regression and packaging | 23 desktop tests on Mac ARM64 and Linux ARM64, 28 packaging tests and packaged Pi QEMU transport smoke passed; superseded by the final older-baseline candidate below |
| Published 1.0.1 older-baseline ARM64 review build | Same DEB passed clean-container install, native/QEMU smoke, four UI exports, reinstall, purge and media preservation on Bookworm, Trixie, Jammy, Noble and Resolute; packaged Pi 5 smoke also passed. 23 desktop and 29 packaging tests passed. [Review release](https://github.com/ahmadexp/homebrew-pc110-atlas/releases/tag/desktop-v1.0.1) published; see [hosted checks and submission status](packaging/pi-apps/SUBMISSION.md) |
